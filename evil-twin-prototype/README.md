...
# 📡 Evil Twin Access Point με Captive Portal
...
![hostapd](https://img.shields.io/badge/hostapd-AP_Mode-blue?style=flat-square)
![dnsmasq](https://img.shields.io/badge/dnsmasq-DHCP/DNS-orange?style=flat-square)
![iptables](https://img.shields.io/badge/iptables-NAT/Firewall-red?style=flat-square)
...
1. Δημιουργείται ένα ανοιχτό Wi-Fi δίκτυο με όνομα **`GUESS_Guest`**, σχεδιασμένο να μιμείται το εταιρικό δίκτυο επισκεπτών της **GUESS, Inc.**
...
4. Ο χρήστης βλέπει μια πειστική σελίδα σύνδεσης εταιρικού guest network με branding GUESS (λευκό φόντο, κόκκινο τριγωνικό λογότυπο, `GUESS?` wordmark) και επιλογές σύνδεσης μέσω **Google**, **Microsoft**, **Apple** ή ως **επισκέπτης** (ονοματεπώνυμο + email).
...
| Captive Portal (Landing) | Google Login | Google Password |
|:---:|:---:|:---:|
| ![Landing Page](./EVIL-TWIN-IMAGES/EVIL%20TWIN%20LANDING%20PAGE.png) | ![Google Login](./EVIL-TWIN-IMAGES/GOOGLE%20LOGIN.png) | ![Google Password](./EVIL-TWIN-IMAGES/GOOGLE%20PASSWORD.png) |

| Σύνδεση στο δίκτυο | Καταγραφή στον Server |
|:---:|:---:|
| ![Network Connect](./EVIL-TWIN-IMAGES/NETWORK%20CONNECT.png) | ![Server Capture](./EVIL-TWIN-IMAGES/SERVER%20CAPTURE.png) |
...
| Βήμα | Τι συμβαίνει |
|------|-------------|
| 1 | Η συσκευή συνδέεται στο `GUESS_Guest`. |
...
| 8 | Η συσκευή επαναλαμβάνει το probe — αυτή τη φορά όμως η κίνησή της διαφεύγει του PREROUTING, φτάνει στους πραγματικούς διακομιστές Google/Apple/Microsoft, λαμβάνει κανονική απάντηση HTTP 204 και η ένδειξη «Χωρίς πρόσβαση στο διαδίκτυο» εξαφανίζεται. |
...
Ο `server.js` εκτελεί δυναμικά εντολές `iptables` ώστε να αποδώσει internet σε κάθε χρήστη που αυθεντικοποιείται. Για να λειτουργεί αυτό χωρίς prompt κωδικού:
...
- Απορρίπτει HTTPS (`TCP 443`) με `TCP RST` για μη-αυθεντικοποιημένες συσκευές, ώστε το HTTPS probe των Android να αποτυγχάνει ακαριαία και η ειδοποίηση captive portal να εμφανίζεται αμέσως
...
| Ρύθμιση δικτύου | hostapd | dnsmasq | server.js |
|:---:|:---:|:---:|:---:|
| ![Setup Network](./EVIL-TWIN-IMAGES/SETUP_NETWORK.png) | ![Setup hostapd](./EVIL-TWIN-IMAGES/SETUP%20HOSTAPD.png) | ![Setup dnsmasq](./EVIL-TWIN-IMAGES/SETUP%20DNSMASQ.png) | ![Setup Server](./EVIL-TWIN-IMAGES/SETUP%20SERVER.png) |
...
Τα σύγχρονα λειτουργικά συστήματα ελέγχουν τη συνδεσιμότητά τους αμέσως μετά τη σύνδεση σε Wi-Fi. Το παρόν πρωτότυπο χρησιμοποιεί **δύο παράλληλους μηχανισμούς** για να εξασφαλίσει την αυτόματη εμφάνιση του portal σε όλες τις συσκευές:
...
> **Σημείωση αρχιτεκτονικής:** Τα παραπάνω domains **δεν δεσμεύονται στο DNS level** — επιστρέφουν τις πραγματικές IP διευθύνσεις των Google/Apple/Microsoft. Ωστόσο, καθώς οι κανόνες `iptables` ανακατευθύνουν ολόκληρη την HTTP κίνηση (`TCP 80`) στον τοπικό Node.js server, τα probes παγιδεύονται ανεξαρτήτως προορισμού. Η προσέγγιση αυτή αποφεύγει τα προβλήματα ενός ψεύτικου HTTPS server με self-signed πιστοποιητικό: τα σύγχρονα Android θεωρούν την αποτυχία TLS επικύρωσης ως «ύποπτο / κατεστραμμένο δίκτυο» και παραμένουν μόνιμα σε κατάσταση «no internet» αντί να εμφανίσουν το portal.
...
Έτσι η επόμενη επανάληψη του probe δεν παγιδεύεται τοπικά: φτάνει στους πραγματικούς διακομιστές, λαμβάνει κανονική απάντηση **HTTP 204** και η ένδειξη «Χωρίς πρόσβαση στο διαδίκτυο» εξαφανίζεται. Το λειτουργικό σύστημα μεταβαίνει σε κατάσταση κανονικής σύνδεσης, χωρίς κανένα προειδοποιητικό banner.
...
> Το παρόν έργο υλοποιήθηκε αποκλειστικά στο πλαίσιο **ακαδημαϊκής έρευνας**, για την ανάλυση ευπαθειών ασύρματων δικτύων και τεχνικών κοινωνικής μηχανικής σε κινητές συσκευές. Η χρήση του επιτρέπεται **μόνο σε ελεγχόμενο εργαστηριακό περιβάλλον**, με **ρητή άδεια** του ιδιοκτήτη του δικτύου-στόχου.
